...
matrix_file <- "raw_files/Data_Lung/Data_Zilionis2019_Lung/Exp_data_UMIcounts.mtx"
genes_file <- "raw_files/Data_Lung/Data_Zilionis2019_Lung/Genes.txt"
barcodes_file <- "raw_files/Data_Lung/Data_Zilionis2019_Lung/Cells.csv"
...
seurat_obj <- CreateSeuratObject(counts = counts, project = "Zilionis et al. 2019", min.cells = 3, min.features = 200)
...
FeaturePlot(seurat_obj, features = "EGFR")

DotPlot(seurat_obj, features = c("COL11A1", "G6PD", "FGF4"), group.by = "cell_type") + 
  RotatedAxis() + 
...
write_rds(seurat_obj, "annotated_rds/zilionis2019_lung.rds")

#slower, but less space consuming
#saveRDS(seurat_obj, "annotated_rds/bischoff2021_lung.rds")

# Get the list of genes
genes <- rownames(seurat_obj)

# Print the first few genes
head(genes); tail(genes)

# If you want to save the list to a file
write.csv(genes, "annotated_rds/genes_zilionis2019_lung.csv", row.names = FALSE)